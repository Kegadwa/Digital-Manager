...
import { useProjects, useTasks, taskProgress } from "@/store/useAppStore";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        <Card className="lg:col-span-1 shadow-card border-border/60">
          ...
          <CardContent className="space-y-2">
            {projectsList.length === 0 && <p className="text-sm text-muted-foreground py-8 text-center">No projects yet</p>}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected?.id === p.id ? "border-primary bg-primary/5" : "border-transparent hover:bg-muted/50"
                  )}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{tCount} task{tCount !== 1 && "s"}</p>
                  </div>
                  <Badge variant="secondary" className={cn("text-[10px]", statusColors[p.status])}>{p.status.replace("_", " ")}</Badge>
                </button>
              );
            })}
          </CardContent>
              ...
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <CardTitle className="text-xl font-display">{selected.name}</CardTitle>
                    {selected.description && <CardDescription className="mt-1">{selected.description}</CardDescription>}
                    <div className="flex flex-wrap items-center gap-2 mt-3 text-xs text-muted-foreground">
                      <Badge variant="secondary" className={cn(statusColors[selected.status])}>{selected.status.replace("_", " ")}</Badge>
                      {selected.dueDate && <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />Due {format(new Date(selected.dueDate), "MMM d, yyyy")}</span>}
                      <span>· Created {formatDistanceToNow(new Date(selected.createdAt))} ago</span>
                    </div>
                  </div>
